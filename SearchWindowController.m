        ...
        if ([btnSearchOriginal isHidden])
        {
            ...
            for (x=0; x<AddrCount ; x++)
            {
                ...
                if ([AttachedMemory loadDataForAddress:Address Buffer:DataBuffer BufLength:BufSize])
                {
                    ...
                    {
                        case 0: // byte
                            [MyAppAddr setValue:[NSString stringWithFormat:@"%hu",(int8_t)(0x00FF & bVal)]];
                            break;
                        case 1: // int16
    ...
    else
    {
        ...
        if (([textSearchValue intValue] == 0) && ([popupDataType indexOfSelectedItem] < 6))
        {
            NSAlert *MyAlert =[NSAlert alertWithMessageText:@"Searching for 0 is a bad idea"
									...
									alternateButton:@""
                                    otherButton:@""
                                    informativeTextWithFormat:@"This version of iHaxGamez will not allow searching for zero. In the next release, (version 2.0) this and many more functions will be included. Stay tuned!."];
