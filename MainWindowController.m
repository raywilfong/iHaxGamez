...
- (void)resetProcessList
{
    ...
    FoundPSN.highLongOfPSN = 0;
    FoundPSN.lowLongOfPSN = kNoProcess;
    
    ProcessInfoRec FoundProcessInfo;
    const int NameBufferLength = 1024;
    char ProcNameBuffer[NameBufferLength];
    FoundProcessInfo.processInfoLength = sizeof(ProcessInfoRec);
    FoundProcessInfo.processName = (StringPtr)&ProcNameBuffer;
#if __LP64__
    FoundProcessInfo.processAppRef = NULL;
#else
    FoundProcessInfo.processAppSpec = NULL;
#endif
    int LoopX;
    pid_t FoundPID;
    while (GetNextProcess(&FoundPSN) == 0)
    {
        if ((FoundPSN.lowLongOfPSN != CurrentAppPSN.lowLongOfPSN) || (FoundPSN.highLongOfPSN != CurrentAppPSN.highLongOfPSN))
        {
            // clear the string
            for (LoopX=0; LoopX<NameBufferLength ; LoopX++)
            {
                ProcNameBuffer[LoopX] = (char)0;
            }
            
            // get the process information
            if (GetProcessInformation(&FoundPSN,&FoundProcessInfo) == 0)
            {
                GetProcessPID(&FoundPSN,&FoundPID);
                ...
                [popupProcessList addItemWithTitle:[NSString stringWithFormat:@"%s (%u)",FoundProcessInfo.processName + 1,FoundPID]];
                [[popupProcessList lastItem] setTag:FoundPID];
                if (PrevSelectedPid == FoundPID)
                {
...
- (IBAction)btnSearchAction:(id)sender
{
    ...
    pid_t SelectedPid = (pid_t)[[popupProcessList itemAtIndex:[popupProcessList indexOfSelectedItem]] tag];
    SearchWindowController *MySearchWindow = [[[SearchWindowController alloc] initWithAppName:MyTitle PID:SelectedPid] autorelease];
